...
export function notificationMatchesFilter(n: AppNotificationItem, f: NotificationFilter): boolean {
...
export function formatLessonWhen(startIso: unknown, endIso: unknown, locale: string): string {
  if (startIso == null || typeof startIso !== 'string') return '';
  try {
    const s = new Date(startIso);
    if (Number.isNaN(s.getTime())) return String(startIso);
    const datePart = s.toLocaleString(locale, { dateStyle: 'medium', timeStyle: 'short' });
...
export function resolveNotificationText(n: AppNotificationItem, t: TFunction, locale: string): { title: string; body: string } {
  const raw = n.context || {};
  ...
  const interp: Record<string, string> = {
    ...
    assignmentTitle: raw.assignmentTitle != null ? String(raw.assignmentTitle) : '',
  };
  if (n.kind) {
...
export function notificationActionLabel(category: string | null | undefined, t: TFunction): string {
