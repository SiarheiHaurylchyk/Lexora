...
export type LessonStrokeTool = 'pen' | 'eraser' | 'line' | 'rect' | 'ellipse' | 'arrow' | 'diamond' | 'text';
...
const TOOLS: LessonStrokeTool[] = ['pen', 'eraser', 'line', 'rect', 'ellipse', 'arrow', 'diamond', 'text'];
...
function sanitizeStroke(raw: Record<string, unknown>): LessonStroke | null {
  if (!raw || typeof raw.id !== 'string' || typeof raw.tool !== 'string' || !Array.isArray(raw.points)) {
    return null;
  }
...
export function decodeLessonBoardWire(raw: string): LessonBoardState | null {
  try {
    const o = JSON.parse(raw) as { v?: number; lessonBoard?: Record<string, unknown> };
    if (!o || (o.v !== 2 && o.v !== 3) || !o.lessonBoard || !Array.isArray(o.lessonBoard.strokes)) {
      return null;
    }
