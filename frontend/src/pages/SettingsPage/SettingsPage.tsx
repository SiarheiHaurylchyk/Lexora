...
import { ClassroomRecordingSettings } from '@/features/ClassroomRecordingSettings';
import { TeacherProfileForm } from '@/features/TeacherProfileForm';
import { VoiceSettings } from '@/features/VoiceSettings';
...
const labelClasses = tw`mb-1.5 block text-[13px] text-text2`;

const tabBase = tw`whitespace-nowrap rounded-[10px] border-0 bg-transparent px-4 py-2.5 text-[13px] font-medium text-text2 cursor-pointer`;
...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    user?.cefrLevel,
    user?.learningGoalType,
    user?.learningGoalWeeks,
    user?.learningGoalNotes,
  ]);
  ...
  const saveLearningGoals = async () => {
    ...
    try {
      const { data } = await authApi.patchProfile({
        ...
        ...(weeksPayload !== undefined
          ? { learningGoalWeeks: weeksPayload }
          : {}),
  ...
  const tabs: { id: TabId; label: string }[] = [
    ...
    ...(canTeach
      ? [{ id: 'teacher' as TabId, label: t('settings.tabs.teacher') }]
      : []),
  ];

  return (
    <div className='box-border w-full py-10'>
      <h1 className='font-display mb-1.5 text-[32px]'>{t('settings.title')}</h1>
      <p className='text-text2 mb-8'>{t('settings.subtitle')}</p>
      ...
      >
        {tabs.map((tabItem) => (
          <button
            ...
            className={cn(tabBase, tab === tabItem.id && tabActive)}
      ...
      {tab === 'account' && (
        <>
          <section className={sectionClasses}>
            ...
            <p className={sectionHelpClasses}>
              {t('settings.account.photoHelp')}
            </p>
            <AvatarUpload />
          </section>
          ...
          <section className={sectionClasses}>
            ...
            <div className='mb-4'>
              <label className={labelClasses}>
                {t('settings.account.displayLabel')}
              </label>
              <input
          ...
          <section className={sectionClasses}>
            <h2 className={sectionTitleClasses}>
              {t('settings.account.learningLanguageTitle')}
            </h2>
            <p className={sectionHelpClasses}>
              {t('settings.account.learningLanguageHelp')}
            </p>
            <div className='mb-4'>
              <label className={labelClasses} htmlFor='settings-learning-lang'>
          ...
          <section className={sectionClasses}>
            <h2 className={sectionTitleClasses}>
              {t('settings.account.goalsTitle')}
            </h2>
            <p className={sectionHelpClasses}>
              {t('settings.account.goalsHelp')}
            </p>
            <div className='mb-4'>
              <label className={labelClasses} htmlFor='settings-cefr'>
            ...
            <div className='mb-4'>
              <label className={labelClasses} htmlFor='settings-goal-type'>
            ...
            <div className='mb-4'>
              <label className={labelClasses} htmlFor='settings-goal-weeks'>
            ...
            <div className='mb-4'>
              <label className={labelClasses} htmlFor='settings-goal-notes'>
          ...
          {!canTeach && (
            <section className={sectionClasses}>
              <h2 className={sectionTitleClasses}>
                {t('settings.account.becomeTitle')}
              </h2>
              <p className={sectionHelpClasses}>
                {t('settings.account.becomeHelp')}
              </p>
              <button
                type='button'
              ...
              >
                <span
                  className='text-brand-light mr-1 inline-flex align-middle'
                  aria-hidden
                >
