...
export function useAiNeuralTts(
  ...
  const speak = useCallback(
    async (text: string, hintLang: string) => {
      ...
      const info = detectTextLanguage(trimmed, {
        hint: hintLang,
        fallback: 'en',
      });
      ...
      if (!neuralEnabled) {
        logTtsRoute({
          ...
          hint: 'API /ai/status returned no neural TTS (xaiTts, groqTts both false). Russian uses OS/browser voices here.',
          primary,
          secondary,
          mixed,
        });
      ...
      if (groqEnglishOnly) {
        ...
        logTtsRoute({
          ...
          hint: 'Groq Orpheus has EN + AR models only; Russian (and other langs) fall back to SpeechSynthesis. Set XAI_API_KEY for neural Russian.',
          primary,
          secondary,
          mixed,
        });
      ...
      const ruEnForSegmentation =
        mixed && isRuEnPair(primary, secondary)
          ? true
          : hasSignificantRuEnLetterMix(trimmed);
      const ruEnPieces =
        multilingualServer && ruEnForSegmentation
          ? splitMixedRuEn(trimmed)
          : [];
      ...
      async function playOneServerWav(
        text2: string,
        lang2: string,
      ): Promise<void> {
      ...
      async function tryServerTts(
        ...
        try {
          ...
          if (
            mixed2 &&
            isRuEnPair(lang2 as DetectedLang, secondary2) &&
            browserSpeakMixedRuEn
          ) {
...
function isRuEnPair(
  a: DetectedLang | null | undefined,
  b: DetectedLang | null | undefined,
): boolean {
  if (!a || !b) return false;
