 ...
 */
export function resolveBrowserSttLang(
  speechLang: string,
  uiLocale?: string,
): string {
...
function pickBestVoiceForLang(
  ...
  const score = (v: SpeechSynthesisVoice): number => {
    ...
    if (/google|microsoft|apple|premium|enhanced|neural|natural/.test(blob))
      s += 4;
    if (
      (prefix === 'ru' || prefix === 'uk') &&
      /irina|dmitri|dmitry|pavel|ekaterina|katya|milena|filipp|boris/.test(blob)
    ) {
      s += 2;
    }
    return s;
 ...
 */
export function splitMixedRuEn(
  text: string,
): { text: string; lang: 'ru' | 'en' }[] {
...
export function useSpeech() {
  ...
  const speak = useCallback(
    (text: string, lang = 'en', opts: SpeakOptions = {}) => {
      ...
      let rate =
        opts.rate ??
        (opts.slow ? speechSettings.slowRate : speechSettings.normalRate);
      if (opts.rate == null && !opts.slow)
        rate = naturalRateForLang(lang, rate);
