...
export function primaryLanguageOf(
  text: string,
  opts: DetectOptions = {},
): DetectedLang {
 ...
 */
export function hasSignificantRuEnLetterMix(
  text: string,
  minEach = 8,
): boolean {
  const scripts = countScripts(text);
 ...
 */
function pickLatinLang(
  hint: string | undefined,
  fallback: DetectedLang,
): DetectedLang {
  const h = (hint || '').trim().toLowerCase();
  const latinLangs: DetectedLang[] = [
    'en',
    'de',
    'es',
    'fr',
  ...
  const latinFallback: DetectedLang = latinLangs.includes(fallback)
    ? fallback
    : 'en';
