...
export interface ModeResult {
  ...
  given?: string;
  /**
   * Сколько раз карточка была показана (только для режима «Карточки»).
   * 1 — ответил с первого раза, 2+ — была помечена «Ещё раз» и повторена.
   * Используется для статистики «среднее попыток» на экране итогов.
   */
  attempts?: number;
}
