 ...
 * No real file upload — keeps the backend stateless and the deployment simple.
 */
const SIZE = 256;
const MAX_FILE_BYTES = 6 * 1024 * 1024; // 6 MB before resize is comfortable.
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.previewWrap}>
        {user.avatarUrl ? (
          <img src={user.avatarUrl} alt="" className={styles.preview} />
        ) : (
          <div className={`${styles.preview} ${styles.previewFallback}`}>{initial}</div>
        )}
      </div>
      ...
      <p className={styles.hint}>{t('settings.avatar.hint')}</p>
    </div>
  );
}

/**
 * Read a File and return a JPEG data URL of size×size pixels (cover crop).
 */
