...
export function hitResizeHandle(
  ...
  const inSq = (cx: number, cy: number) => Math.abs(wx - cx) <= half && Math.abs(wy - cy) <= half;
  const mx = (b.l + b.r) / 2;
  const my = (b.t + b.b) / 2;
...
export function resizeHandleCenters(b: { l: number; t: number; r: number; b: number }): [ResizeHandle, number, number][] {
  const mx = (b.l + b.r) / 2;
  const my = (b.t + b.b) / 2;
  return [
    ['nw', b.l, b.t],
...
function mapLin(v: number, a0: number, a1: number, b0: number, b1: number): number {
  if (Math.abs(a1 - a0) < 1e-9) return b0;
  return b0 + ((v - a0) * (b1 - b0)) / (a1 - a0);
}

function clampBox(L: number, T: number, R: number, B: number): { L: number; T: number; R: number; B: number } {
  let L2 = L;
  let T2 = T;
  let R2 = R;
  let B2 = B;
...
export function getTightBounds(st: LessonStroke, ctx: CanvasRenderingContext2D): { l: number; t: number; r: number; b: number } | null {
  const p = st.points;
  if (st.tool === 'text') {
  ...
  if (
    p.length >= 4 &&
    (st.tool === 'line' || st.tool === 'rect' || st.tool === 'ellipse' || st.tool === 'arrow' || st.tool === 'diamond')
  ) {
    return {
      l: Math.min(p[0], p[2]),
      r: Math.max(p[0], p[2]),
...
function mapPenToBox(
  ...
  for (let i = 0; i < points.length; i += 2) {
    out.push(mapLin(points[i], oL, oR, nL, nR), mapLin(points[i + 1], oT, oB, nT, nB));
  }
  return out;
}
...
export function resizeStrokeFromHandle(
  ...
  if (origin.tool === 'line' || origin.tool === 'rect' || origin.tool === 'ellipse' || origin.tool === 'arrow' || origin.tool === 'diamond') {
