...
function distPointSegment(px: number, py: number, x1: number, y1: number, x2: number, y2: number): number {
  const dx = x2 - x1;
  const dy = y2 - y1;
...
function ellipseMetric(x0: number, y0: number, x1: number, y1: number, px: number, py: number): number {
  const cx = (x0 + x1) / 2;
  const cy = (y0 + y1) / 2;
...
function pointInDiamond(px: number, py: number, x0: number, y0: number, x1: number, y1: number): boolean {
  ...
  const my = (top + bot) / 2;
  return pointInTriangle(px, py, mx, top, right, my, mx, bot) || pointInTriangle(px, py, mx, top, left, my, mx, bot);
}

/** ctx должен быть в мировых координатах (translate + scale как при отрисовке доски) */
function textBox(st: LessonStroke, ctx: CanvasRenderingContext2D): { l: number; t: number; r: number; b: number } | null {
...
export function getStrokeBounds(st: LessonStroke, ctx: CanvasRenderingContext2D): { l: number; t: number; r: number; b: number } | null {
  const p = st.points;
  ...
  if (st.tool === 'line' || st.tool === 'rect' || st.tool === 'ellipse' || st.tool === 'arrow' || st.tool === 'diamond') {
    if (p.length < 4) return null;
    const x0 = p[0];
    const y0 = p[1];
    const x1 = p[2];
...
export function hitTestStroke(st: LessonStroke, wx: number, wy: number, viewScale: number, ctx: CanvasRenderingContext2D): boolean {
  ...
  if (st.tool === 'text') {
    const b = textBox(st, ctx);
    if (!b) return false;
    return wx >= b.l - tol && wx <= b.r + tol && wy >= b.t - tol && wy <= b.b + tol;
  }

  if (st.tool === 'pen' || st.tool === 'eraser') {
    if (p.length < 4) return false;
    const half = (st.tool === 'eraser' ? Math.max(st.width * 2.2, 10) : st.width) / 2 + tol;
    for (let i = 0; i < p.length - 2; i += 2) {
      if (distPointSegment(wx, wy, p[i], p[i + 1], p[i + 2], p[i + 3]) <= half) return true;
    }
    return false;
  }
  ...
  if (st.tool === 'arrow') {
    ...
    const bx = x1 - ux * head;
    const by = y1 - uy * head;
    return pointInTriangle(wx, wy, x1, y1, bx + px * spread, by + py * spread, bx - px * spread, by - py * spread);
  }
