...
import type { LessonBoardState, LessonBoardView, LessonLineStyle, LessonStroke, LessonStrokeTool } from '../../lib/lessonBoardModel';
import { cloneLessonBoardState } from '../../lib/lessonBoardModel';
import { getStrokeBounds, pickStrokeAt } from '../../lib/lessonBoardGeometry';
import {
  getTightBounds,
  hitResizeHandle,
  resizeHandleCenters,
  ...
  type ResizeHandle,
} from '../../lib/lessonBoardResize';
import styles from './LessonBoard.module.css';

function withWorldCtx<T>(canvas: HTMLCanvasElement | null, view: LessonBoardView, fn: (ctx: CanvasRenderingContext2D) => T): T | null {
...
const FILL_COLORS = ['', '#3d445466', '#6db3ff44', '#ff5c5c44', '#5cff9d44', '#14161f'];
...
function applyLineDash(ctx: CanvasRenderingContext2D, style: LessonLineStyle | undefined, viewScale: number) {
...
function drawStroke(ctx: CanvasRenderingContext2D, stroke: LessonStroke, viewScale: number) {
  const p = stroke.points;
  const baseW = stroke.tool === 'eraser' ? Math.max(stroke.width * 2.2, 10) : stroke.width;
  ...
  const fill = stroke.fillColor && stroke.fillColor.length > 0 ? stroke.fillColor : null;
  ...
  if (stroke.tool === 'rect') {
    ...
    const rr = stroke.rounded ? Math.min(14 / viewScale, Math.min(w, h) * 0.12) : 0;
    ctx.beginPath();
...
function drawResizeHandles(ctx: CanvasRenderingContext2D, bounds: { l: number; t: number; r: number; b: number }, viewScale: number) {
...
function drawGrid(ctx: CanvasRenderingContext2D, view: LessonBoardState['view'], w: number, h: number) {
  ctx.save();
...
function bboxTools(t: LessonStrokeTool): boolean {
  return t === 'line' || t === 'rect' || t === 'ellipse' || t === 'arrow' || t === 'diamond';
}
...
export default function LessonBoard({ className, state, onChange }: LessonBoardProps) {
  ...
  const resizeVisualRef = useRef<{ id: string; stroke: LessonStroke } | null>(null);
    ...
    (raw: string) => {
      ...
      if (te.editingId) {
        if (!trimmed) {
          onChange((prev) => {
            pushUndoSnapshot(prev);
            return { ...prev, strokes: prev.strokes.filter((s) => s.id !== te.editingId) };
          });
        } else {
          onChange((prev) => {
            pushUndoSnapshot(prev);
            return {
              ...prev,
              strokes: prev.strokes.map((s) => (s.id === te.editingId ? { ...s, text: trimmed } : s)),
            };
          });
        }
      } else if (trimmed) {
  ...
  const startDraft = (x: number, y: number, strokeTool: LessonStrokeTool): LessonStroke => {
    ...
    const closedShape = strokeTool === 'rect' || strokeTool === 'ellipse' || strokeTool === 'diamond';
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (panNow) {
      ...
      panRef.current = { sx: e.clientX, sy: e.clientY, ox: state.view.offsetX, oy: state.view.offsetY };
      return;
    }
      ...
      if (selectedId && canvasRef.current) {
        ...
        if (selBase) {
          const handle = withWorldCtx(canvasRef.current, state.view, (ctx) => {
            const dv = dragVisualRef.current;
            const sd = dv?.id === selBase.id ? { ...selBase, points: dv.points } : selBase;
            const b = getStrokeBounds(sd, ctx);
            if (!b) return null;
          ...
          if (handle) {
            clearSelectDragRefs();
            const dv = dragVisualRef.current;
            const selDraw = dv?.id === selBase.id ? { ...selBase, points: dv.points } : selBase;
            const originStroke = JSON.parse(JSON.stringify(selDraw)) as LessonStroke;
            const tight0 = withWorldCtx(canvasRef.current, state.view, (ctx) => getTightBounds(selDraw, ctx));
            if (tight0) {
              pushUndoSnapshot(state);
              resizeRef.current = { id: selBase.id, handle, originStroke, tight0 };
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (resizeRef.current) {
      ...
      const next = resizeStrokeFromHandle(r.originStroke, r.tight0, r.handle, x, y);
      if (next) {
    ...
    if (tool === 'select') {
      ...
      if (pending && !dragRef.current) {
        const movedPx = Math.hypot(e.clientX - pending.cx, e.clientY - pending.cy);
      ...
      if (dr) {
        ...
        const newPoints = dr.originPoints.map((v, i) => (i % 2 === 0 ? v + dx : v + dy));
        ...
        if (dragFlushRafRef.current == null) {
          dragFlushRafRef.current = requestAnimationFrame(() => {
            ...
            if (dv && did === dv.id) {
              onChange((prev) => ({
                ...prev,
                strokes: prev.strokes.map((s) => (s.id === dv.id ? { ...s, points: dv.points } : s)),
              }));
            }
          });
        }
  ...
  const toolBtn = (next: Tool, label: string, icon: React.ReactNode) => (
    <button
      key={next}
      type="button"
      className={classNames(styles.toolBtn, tool === next && styles.toolBtnActive)}
      aria-label={label}
  ...
  return (
    <div className={classNames(styles.wrap, className)}>
      <header className={styles.topDock}>
        <div className={styles.topDockSpacer} aria-hidden />
        <div className={styles.toolRail} role="toolbar" aria-label={t('classroom.lessonBoard.toolsAria')}>
          {toolBtn('select', t('classroom.lessonBoard.select'), <MousePointer2 size={20} strokeWidth={2} />)}
          {toolBtn('pen', t('classroom.lessonBoard.pen'), <Pencil size={20} strokeWidth={2} />)}
          {toolBtn('text', t('classroom.lessonBoard.text'), <Type size={20} strokeWidth={2} />)}
          {toolBtn('eraser', t('classroom.lessonBoard.eraser'), <Eraser size={20} strokeWidth={2} />)}
          {toolBtn('line', t('classroom.lessonBoard.line'), <Minus size={20} strokeWidth={2} />)}
          {toolBtn('arrow', t('classroom.lessonBoard.arrow'), <ArrowRight size={20} strokeWidth={2} />)}
          {toolBtn('rect', t('classroom.lessonBoard.rect'), <Square size={20} strokeWidth={2} />)}
          {toolBtn('ellipse', t('classroom.lessonBoard.ellipse'), <Circle size={20} strokeWidth={2} />)}
          {toolBtn('diamond', t('classroom.lessonBoard.diamond'), <Diamond size={20} strokeWidth={2} />)}
          <button
            type="button"
            className={classNames(
              styles.toolBtn,
              tool === 'pan' && styles.toolBtnActive,
              spacePanHeld && styles.toolBtnSpacePan,
            )}
        ...
        <div className={styles.topDockActions}>
          <button
            type="button"
          ...
          <button type="button" className={styles.toolBtn} aria-label={t('classroom.lessonBoard.undo')} onClick={handleUndo}>
            <Undo2 size={20} strokeWidth={2} />
          </button>
          <button type="button" className={styles.toolBtn} aria-label={t('classroom.lessonBoard.clear')} onClick={handleClear}>
      ...
      <div className={styles.main}>
        <aside className={styles.propsPanel} aria-label={t('classroom.lessonBoard.propertiesAria')}>
          <div className={styles.roundedRow}>
            <button
              type="button"
              className={classNames(styles.roundedBtn, showGrid && styles.roundedBtnActive)}
              aria-pressed={showGrid}
              aria-label={t('classroom.lessonBoard.canvasGridAria')}
              onClick={() => setShowGrid((g) => !g)}
            >
              <Grid3x3 size={18} strokeWidth={2} className={styles.gridToggleIcon} aria-hidden />
              <span className={styles.roundedLabel}>{t('classroom.lessonBoard.canvasGrid')}</span>
            </button>
          </div>

          <p className={styles.propLabel}>{t('classroom.lessonBoard.stroke')}</p>
          <div className={styles.swatchesRow} role="group" aria-label={t('classroom.lessonBoard.colors')}>
            {STROKE_COLORS.map((c) => (
              <button
                key={c}
                type="button"
                className={classNames(styles.swatch, color === c && styles.swatchActive)}
                style={{ background: c }}
                aria-label={c}
                onClick={() => setColor(c)}
              />
          ...
          <p className={styles.propLabel}>{t('classroom.lessonBoard.fill')}</p>
          <div className={styles.swatchesRow} role="group" aria-label={t('classroom.lessonBoard.fill')}>
            {FILL_COLORS.map((c) => (
              <button
                key={c || 'none'}
                type="button"
                className={classNames(styles.swatch, styles.swatchSm, !c && styles.swatchNone, fillColor === c && styles.swatchActive)}
          ...
          <p className={styles.propLabel}>{t('classroom.lessonBoard.width')}</p>
          <div className={styles.widthRow} role="group" aria-label={t('classroom.lessonBoard.width')}>
            {WIDTHS.map((w) => (
              <button
                key={w}
                type="button"
                className={classNames(styles.widthBtn, width === w && styles.widthBtnActive)}
                onClick={() => setWidth(w)}
              >
                {w}
              </button>
          ...
          <p className={styles.propLabel}>{t('classroom.lessonBoard.lineStyle')}</p>
          <div className={styles.lineStyleRow} role="group" aria-label={t('classroom.lessonBoard.lineStyle')}>
            {(
              [
              ...
              <button
                key={s}
                type="button"
                className={classNames(styles.lineStyleBtn, lineStyle === s && styles.lineStyleBtnActive)}
                aria-label={label}
                aria-pressed={lineStyle === s}
                onClick={() => setLineStyle(s)}
              >
                <span className={styles.lineStyleGlyph} data-style={s} aria-hidden />
              </button>
            ))}
          </div>
          ...
          <div className={styles.roundedRow}>
            <button
              type="button"
              className={classNames(styles.roundedBtn, roundedRect && styles.roundedBtnActive)}
              aria-pressed={roundedRect}
              disabled={tool !== 'rect'}
              onClick={() => setRoundedRect((v) => !v)}
            >
              <span className={styles.roundedIcon} data-rounded={roundedRect} aria-hidden />
              <span className={styles.roundedLabel}>{t('classroom.lessonBoard.roundedCorners')}</span>
            </button>
          </div>

          <label className={styles.opacityBlock}>
            <span className={styles.propLabel}>{t('classroom.lessonBoard.opacity')}</span>
            <input
              type="range"
            ...
            />
            <span className={styles.opacityValue}>{opacityPct}%</span>
          </label>

          {tool === 'text' && (
            <>
              <p className={styles.propLabel}>{t('classroom.lessonBoard.textSize')}</p>
              <div className={styles.widthRow} role="group" aria-label={t('classroom.lessonBoard.textSize')}>
                {TEXT_SIZES.map((fs) => (
                  <button
                    key={fs}
                    type="button"
                    className={classNames(styles.widthBtn, textFontSize === fs && styles.widthBtnActive)}
        ...
        <div ref={hostRef} className={styles.canvasHost}>
          <canvas
            ...
            className={classNames(
              styles.canvas,
              (tool === 'pan' || spacePanHeld) && styles.canvasPan,
              tool === 'text' && styles.canvasText,
              tool === 'select' && styles.canvasSelect,
              (draggingSelection || resizingSelection) && styles.canvasDragging,
            )}
          ...
          <div className={styles.hintBar}>{t('classroom.lessonBoard.canvasHint')}</div>
        </div>
      </div>
    </div>
  );
