...
async function refreshAccessToken(): Promise<string> {
  ...
  refreshPromise = axios
    ...
    .then((res) => {
      ...
      store.dispatch(updateTokens({ accessToken, refreshToken: newRefresh || refreshToken }));
...
export const authApi = {
  ...
  patchProfile: (data: Record<string, unknown>) => api.patch<User>('/auth/profile', data),
...
export const availabilityApi = {
  ...
  cancelBooking: (slotId: number) => api.delete(`/me/availability/${slotId}/booking`),
  /** Public: open slots for a teacher. */
  forTeacher: (teacherId: number, fromIso: string, toIso: string) =>
    api.get(`/teachers/${teacherId}/availability`, { params: { from: fromIso, to: toIso } }),
...
export const classroomsApi = {
  workspace: (linkId: number) => api.get<ClassroomWorkspacePayload>(`/me/classrooms/${linkId}`),
  eligibleLessons: (linkId: number) => api.get<ClassroomLessonOption[]>(`/me/classrooms/${linkId}/eligible-lessons`),
  prepareCall: (linkId: number) => api.post<PrepareCallPayload>(`/me/classrooms/${linkId}/prepare-call`),
  patchRecordingConsent: (linkId: number, data: { consent: boolean }) =>
    api.patch<ClassroomWorkspacePayload>(`/me/classrooms/${linkId}/recording-consent`, data),
  patchActiveLesson: (linkId: number, data: { lessonId: number | null }) =>
    api.patch<ClassroomWorkspacePayload>(`/me/classrooms/${linkId}/active-lesson`, data),
  patchLessonFocus: (
    linkId: number,
    data: {
  ...
  ) => api.patch<ClassroomWorkspacePayload>(`/me/classrooms/${linkId}/lesson-focus`, data),
  lessonFocusSnapshot: (linkId: number) =>
    api.get<ClassroomLessonFocusSnapshot>(`/me/classrooms/${linkId}/lesson-focus`),
  timer: (linkId: number) => api.get<ClassroomTimerPayload>(`/me/classrooms/${linkId}/timer`),
  timerStart: (linkId: number) => api.post<ClassroomTimerPayload>(`/me/classrooms/${linkId}/timer/start`),
  timerStop: (linkId: number) => api.post<ClassroomTimerPayload>(`/me/classrooms/${linkId}/timer/stop`),
  timerReset: (linkId: number) => api.post<ClassroomTimerPayload>(`/me/classrooms/${linkId}/timer/reset`),
};
...
export const deckApi = {
  ...
  updateDeck: (id: number, data: Partial<DeckItem>) => api.put<DeckItem>(`/decks/${id}`, data),
  deleteDeck: (id: number) => api.delete(`/decks/${id}`),
  search: (q: string, page = 0) => api.get(`/decks/search?q=${encodeURIComponent(q)}&page=${page}`),
...
export const shareApi = {
  ...
  listDeckShares: (deckId: number) => api.get<DeckShare[]>(`/decks/${deckId}/shares`),
...
export const studyApi = {
  ...
  recordAnswer: (sessionId: number, data: { cardId: number; correct: boolean; rating: number }) =>
...
export const chatApi = {
  ...
  loadThread: (peerUserId: number) => api.get<ChatThread>(`/chat/${peerUserId}/thread`),
  sendMessage: (
    peerUserId: number,
...
export const lessonsApi = {
  ...
  createLesson: (data: { title: string; summary?: string; studentId?: number | null }) =>
    api.post<LessonItem>('/lessons', data),
  /** Change title / summary / target student. */
  updateLesson: (id: number, data: { title: string; summary?: string; studentId?: number | null }) =>
  ...
  addLessonSection: (lessonId: number, data: { title: string; sortOrder?: number }) =>
    api.post<LessonItem>(`/lessons/${lessonId}/sections`, data),
  updateLessonSection: (lessonId: number, sectionId: number, data: { title: string; sortOrder?: number }) =>
  ...
  deckFromBlocks: (lessonId: number) => api.post<DeckItem>(`/lessons/${lessonId}/deck-from-blocks`),
};
...
export const notificationsApi = {
  ...
  markRead: (id: number) => api.patch<AppNotificationItem>(`/me/notifications/${id}/read`),
  markAllRead: () => api.post('/me/notifications/read-all'),
};

/** Community Materials catalog and personal library bookmarks. */
export const materialsApi = {
  catalog: (params: { page?: number; size?: number; q?: string; sort?: 'popular' | 'new' }) =>
    api.get<Paged<DeckItem>>('/materials/catalog', { params }),
  personal: () => api.get<MaterialsPersonalViewDTO>('/materials/personal'),
  saveCatalogDeck: (deckId: number) => api.post<DeckItem>(`/materials/catalog/${deckId}/save`),
  unsaveCatalogDeck: (deckId: number) => api.delete(`/materials/saved/${deckId}`),
  patchListing: (
    deckId: number,
    body: { listedInMaterialsCatalog?: boolean; catalogPriceCents?: number | null; cefrLevel?: string | null },
...
export const favoritesApi = {
  ...
  remove: (teacherId: number) => api.delete(`/me/favorite-teachers/${teacherId}`),
};
...
export const assignmentsApi = {
  ...
  markComplete: (id: number) => api.patch<StudentAssignmentItem>(`/me/assignments/${id}/complete`),
  patchStudentResponse: (id: number, body: { studentResponse?: string | null }) =>
