 ...
 */
export function useBrowserSpeechRecognition(opts: UseBrowserSpeechRecognitionOptions) {
  ...
  const start = useCallback(() => {
    ...
    try {
      ...
      optsRef.current.onError?.(e instanceof Error ? e.message : 'Could not start microphone');
      recRef.current = null;
      setListening(false);
    }
  }, []);

  return { listening, start, stop, supported: isBrowserSpeechRecognitionSupported() };
}
